...
public class DriverController {
    ...
    public DriverResponse location(@Valid @RequestBody LocationRequest request, Authentication auth) {
        return service.updateLocation(auth.getName(), request);
    }

    @PatchMapping("/status")
    public DriverResponse status(@Valid @RequestBody StatusRequest request, Authentication auth) {
        return service.updateStatus(auth.getName(), request.status());
    }

    @GetMapping("/me")
    public DriverResponse me(Authentication auth) { return service.get(auth.getName()); }

    @GetMapping("/rides")
    public java.util.List<Object> rides(Authentication auth) {
